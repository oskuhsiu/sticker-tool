...
Last verified against the source tree on 2026-08-11.
...
- Browser none/color-key: preserve alpha, or apply one of two explicit deterministic color-key scopes. The safe default selects candidates four-way connected to the outer edge; enclosed matching regions remain RGBA-identical, while enclosed background holes may remain. Its edge handling is decontaminated soft matte by default, with soft matte and hard alternatives. The opt-in whole-image scope instead compares every pixel to one target RGB with Chebyshev tolerance from 0.0% through 20.0% and hard-removes matches; 0.0% means exact RGB only, and matching subject pixels are intentionally removed. Video editors preview up to three time-uniform initial target-frame candidates live before rerendering. Auto-detected green screens retain their dedicated greenness matte only in edge-connected mode. None and semantic removers neither receive nor include color-key options in cache identity.
...
Local browser-decodable video
  ...
  -> save/reopen a strict, checksummed Project ZIP V6 at any time
```

V6 has no sampling-count control. `frameCoverage='all-presentation-frames'` means every decoded sample
...
`VideoGridPlan.rects` drives preflight, every raw-master crop, and the Project ZIP V6 grid. Grid editing is
...
Project ZIP V6 adds the intentional whole-image color-key scope and bounded tolerance to the V5 output-target, Popup static-frame, and edge-setting contract. It includes only declared manifest/report, `master/`,
...
audio, secrets, and the optional model cache. V5 edge-connected settings migrate losslessly to V6. V4 imports strip the retired scope field: edge-connected current renders remain valid, while legacy all-matching current renders are verified and then cleared for safe rerender. V3 implicit global/soft color-key renders receive the same invalidation and are never aliased to the new whole-image mode; V2 imports migrate to Animated Sticker. V1 imports are explicitly tagged
...
connectivity and decontamination. Users may instead opt into whole-image hard removal with a 0.0–20.0%
tolerance after inspecting representative initial target-frame candidates, or switch the connected-mode
edge treatment to soft or hard.
