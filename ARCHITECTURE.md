...
- Browser none/color-key: preserve alpha, or apply a deterministic color key only to candidates that are four-way connected to the outer edge. Matching regions enclosed by non-background pixels remain RGBA-identical, so matching subject details are protected and enclosed background holes may remain. Edge handling is decontaminated soft matte by default, with soft matte and hard removal alternatives; hard removal can produce jagged or shrunken contours. Auto-detected green screens use the same fixed connectivity with their dedicated greenness matte and bounded despill. None and semantic removers neither receive nor include color-key edge options in cache identity.
...
Local browser-decodable video
  ...
  -> save/reopen a strict, checksummed Project ZIP V5 at any time
```

V5 has no sampling-count control. `frameCoverage='all-presentation-frames'` means every decoded sample
...
`VideoGridPlan.rects` drives preflight, every raw-master crop, and the Project ZIP V5 grid. Grid editing is
...
Project ZIP V5 adds the immutable output target, Popup static-frame selections, and explicit color-key edge settings, and includes only declared manifest/report, `master/`,
...
audio, secrets, and the optional model cache. V4 imports strip the retired scope field: edge-connected current renders remain valid, while all-matching current renders are verified and then cleared for safe rerender. V3 implicit global/soft color-key renders receive the same invalidation; V2 imports migrate to Animated Sticker. V1 imports are explicitly tagged
...
Solid-color keying is disabled by default for Video. When selected, it always uses four-way outer-edge
connectivity and defaults to decontamination. Users may switch the edge treatment to soft or hard,
accepting a possible background halo or jagged/shrunken contours respectively.
...
the adapter supplies those fields. Regular Emoji, Video V5, and Pop-up Sticker mode supply evidence
...
Video V5 adapters represent decoded duration in `ImageInfo`; older regular animation adapters may still
omit it.
