...
- Browser none/color-key: preserve alpha, or apply a deterministic color key with two color-key-only axes. Scope is either four-way outer-edge connected or all matching candidates. Edge handling is soft matte, decontaminated soft matte, or hard removal. The new default is outer-edge connected plus decontamination; all-matching can clear enclosed holes but can also punch through matching subject details, while hard removal can produce jagged/shrunken contours. Auto-detected green screens use the same scope/edge contract with the dedicated greenness matte and bounded despill. None and semantic removers neither receive nor include these options in cache identity.
...
Local browser-decodable video
  ...
  -> save/reopen a strict, checksummed Project ZIP V4 at any time
```

V4 has no sampling-count control. `frameCoverage='all-presentation-frames'` means every decoded sample
...
`VideoGridPlan.rects` drives preflight, every raw-master crop, and the Project ZIP V4 grid. Grid editing is
...
Project ZIP V4 adds the immutable output target, Popup static-frame selections, and explicit color-key scope/edge settings, and includes only declared manifest/report, `master/`,
...
audio, secrets, and the optional model cache. V3 color-key settings migrate to historical all-matching/soft semantics; V2 imports migrate to Animated Sticker. V1 imports are explicitly tagged
...
Solid-color keying is disabled by default for Video. When selected, its new default is outer-edge connected
plus decontamination. Users may explicitly switch to all-matching to clear enclosed holes, accepting that
matching subject details can be removed, or to hard edges, accepting jagged or shrunken contours.
...
the adapter supplies those fields. Regular Emoji, Video V4, and Pop-up Sticker mode supply evidence
...
Video V4 adapters represent decoded duration in `ImageInfo`; older regular animation adapters may still
omit it.
