...
the advanced control can explicitly preserve original colors or set a palette ceiling. Background removal is lazy: one draft prepares a session from up to three time-stratified unique raw visuals, and preview plus final candidates share its calibration identity. Automatic removal is cached in a bounded session LRU independently of Keep-mask composition, so painting does not rerun a semantic model. The editor marks corrected raw visuals in text and can explicitly copy one equal-size mask to the current range as fixed coordinates—not motion tracking. The **`選取 raw visual`** thumbnail selector starts collapsed while the active correction editor remains visible. Before a fresh render it shows only the deterministic time-uniform planned picks for **`目標格數`**; after rendering it shows the actual final picks, including deterministic replacements. Repeated presentation samples that share one raw visual produce one correction chip. This is only a view filter: the complete raw timeline remains available for removal calibration, correction copy-to-range, duplicate/replacement processing, and Project ZIP data. Unused frames are tried only when removal or quantization makes adjacent selected visuals identical; an over-budget result is reported without triggering background removal on the remaining range. Exact-target encoding may reduce colors but
...
output count preserves edited outer bounds and internal separators. It updates the planned thumbnail picks but does not auto-fit clip lines. Those shared guides define one row-major layout for
