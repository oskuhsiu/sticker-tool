...
> 狀態：第一階段的外框連通／全圖範圍與三種邊緣策略已在 Web 實作；互動式保留／去除修正仍是後續提案。零提示模型比較已加入實驗性 Colab Notebook。適用於背景接近純色、帶有少量雜訊或壓縮色差，但主體、文字或特效也可能包含相同顏色的圖片與影片。

## TL;DR

- 單色色鍵現在預設只刪除顏色接近背景、而且能以四向路徑連到畫面外框的區域。
- 使用者可改選「全圖相近色」清除封閉背景洞，但這會重新承擔挖空主體同色部分的風險。
- 邊緣可選「清除色暈」（預設）、「柔和邊緣」或「硬邊」；硬邊不留 soft matte，代價是鋸齒或輪廓縮小。
- 如果主體同色區域直接碰到背景，且中間沒有任何可辨認的色差或邊界，單靠像素無法可靠判斷，必須使用保留／去除筆畫、前景框選或乾淨背景參考圖。

## 問題

舊版 Web 的 `chromaKeySolid` 使用固定背景色，計算每個像素與背景色的 Chebyshev 距離：距離不超過 20 時完全透明，20–64 之間產生漸層 alpha。它把這個判斷套到全圖，因此主體內只要出現相近顏色，也會一起被去除。現在同一候選規則增加了明確的外框連通／全圖範圍選項。
...
第一版保持本機、確定性且不依賴新模型。平台中立的選項契約位於 `src/core/colorKey.ts`；像素 Raster 實作留在 Web adapter。這些選項只屬於單色色鍵，不會顯示或傳給 IMG.LY、BiRefNet、Colab 或不去背模式。

## 目前 UI

- 去背範圍：`外框連通（保護主體）`／`全圖相近色（可能挖空主體）`
- 邊緣處理：`清除色暈（建議）`／`柔和邊緣（可能留背景圈）`／`硬邊（可能鋸齒）`
- 背景取樣：工作流既有的自動偵測、色彩輸入或手動點圖取色
- 後續增強：即時遮罩預覽、`保留筆刷`與`去除筆刷`
