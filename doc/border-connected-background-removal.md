...
> 狀態：邊界連通／互動修正仍是尚未實作的設計提案；零提示模型比較已加入實驗性 Colab Notebook。適用於背景接近純色、帶有少量雜訊或壓縮色差，但主體、文字或特效也可能包含相同顏色的圖片與影片。

## TL;DR

- 現有單色色鍵會刪除全圖所有接近背景色的像素，因此無法保證主體中的同色部分安全。
- 建議新增「邊界連通去背」：只刪除顏色接近背景、而且能一路連通到畫面邊界的區域。
- 被主體輪廓包住的同色區域會保留；背景略有色差時可用容差與羽化處理。
- 如果主體同色區域直接碰到背景，且中間沒有任何可辨認的色差或邊界，單靠像素無法可靠判斷，必須使用保留／去除筆畫、前景框選或乾淨背景參考圖。

## 問題

目前 Web 的 `chromaKeySolid` 使用固定背景色，計算每個像素與背景色的 Chebyshev 距離：距離不超過 20 時完全透明，20–64 之間產生漸層 alpha。這能容忍近純色背景的少量雜訊，但判斷是全域的；主體內只要出現相近顏色，也會一起被去除。
...
5. 對最終邊界建立窄的 unknown band，做 1–3 px 羽化並乘回來源 alpha，避免鋸齒與硬邊。

第一版應保持本機、確定性且不依賴新模型。核心判斷可放在平台中立的 `src/core/`；Raster／Canvas 轉換仍留在 CLI 與 Web adapter。
...
- 顏色容差：控制背景雜訊的可接受範圍
- 邊緣柔化：控制窄邊界的羽化寬度
- 即時遮罩預覽：以半透明顏色顯示將被移除的區域
- 後續增強：`保留筆刷`與`去除筆刷`，作為演算法無法判斷時的少量提示
