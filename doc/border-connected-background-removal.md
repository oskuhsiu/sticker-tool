...
> 狀態：安全預設仍是外框四向連通與三種邊緣策略；外框色碼容差可在 0%–200% 調整，100% 保留既有自動學習基準。Web 另提供明確 opt-in 的全圖色碼模式，以獨立的 0.0%–20.0% 容差硬挖所有符合像素。Video 編輯器會用最多三張代表候選 frame 即時預覽。互動式保留／去除修正仍是後續提案。零提示模型比較已加入實驗性 Colab Notebook。

## TL;DR

- 預設的單色色鍵只刪除顏色接近背景、而且能以四向路徑連到畫面外框的區域。
- 「外框色碼容差」以 1% 為 step，在 0%–200% 間縮放自動學習的確定背景與過渡區門檻；100% 是既有預設，降低會收窄、提高會放寬，但不會改成全圖刪除。
...
外框容差是自動學習門檻的倍率，不是另一套固定 RGB 距離：100% 使用既有學習結果，較低值同步收窄確定背景與過渡區門檻，較高值同步放寬。無論倍率為何，只有符合條件且能以四向路徑連到外框的像素會被移除。舊 V7 Project 若沒有此欄位，會按 100% 解讀。

## 目前 UI

- 去背範圍：`外框連通（安全預設）`／`全圖色碼（符合就挖掉）`
- 外框連通的邊緣處理：`清除色暈（建議）`／`柔和邊緣（可能留背景圈）`／`硬邊（可能鋸齒）`
- 外框色碼容差：`0%–200%`、step `1%`；`100%` 是自動學習基準
- 全圖色碼容差：`0.0%–20.0%`、step `0.1%`，並提供 `−0.1%`／`+0.1%` 微調按鈕
- 組合模式：外框與全圖容差分開設定，互不取代
- 背景取樣：工作流既有的自動偵測、色彩輸入或手動點圖取色
- Video 即時預覽：最多三張初選代表 frame，可切換查看處理前／處理後與挖除 pixel 數
- Video 的 `選取 raw visual` 預設收合，但目前作用中的修正編輯器仍顯示。尚未產生新成品時，只顯示依 `目標格數` 做時間均勻預選的 chip；成品產生後改顯示實際最終選取（含確定性替補）。多個 presentation sample 若共用同一 raw visual，只顯示一個修正 chip。
- 上述 chip 清單只過濾畫面：完整 raw timeline 仍供校準、修正複製到範圍、重複／替補處理與 Project ZIP 資料使用。變更 `目標格數` 只更新預選 chip，未新增 clip line auto-fit。
- 後續增強：`保留筆刷`與`去除筆刷`
...
- 外框容差 100% 必須與既有自動學習結果一致；降低／提高時，四向連通候選範圍應相應收窄／放寬。
