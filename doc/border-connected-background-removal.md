...
> 狀態：第一階段的外框四向連通與三種邊緣策略已在 Web 實作；會挖空主體的全圖相近色已停用，互動式保留／去除修正仍是後續提案。零提示模型比較已加入實驗性 Colab Notebook。適用於背景接近純色、帶有少量雜訊或壓縮色差，但主體、文字或特效也可能包含相同顏色的圖片與影片。

## TL;DR

- 單色色鍵只刪除顏色接近背景、而且能以四向路徑連到畫面外框的區域。
- 封閉的背景洞可能保留；在沒有額外提示時，工具不會用全圖扣色猜測它是背景，以免挖空主體同色部分。
- 邊緣可選「清除色暈」（預設）、「柔和邊緣」或「硬邊」；硬邊不留 soft matte，代價是鋸齒或輪廓縮小。
- 如果主體同色區域直接碰到背景，且中間沒有任何可辨認的色差或邊界，單靠像素無法可靠判斷，必須使用保留／去除筆畫、前景框選或乾淨背景參考圖。

## 問題

舊版 Web 的 `chromaKeySolid` 使用固定背景色，計算每個像素與背景色的 Chebyshev 距離：距離不超過 20 時完全透明，20–64 之間產生漸層 alpha。它把這個判斷套到全圖，因此主體內只要出現相近顏色，也會一起被去除。現在同一候選規則固定使用外框四向連通；舊 Project 的全圖相近色成品不會被當成安全成品繼續使用。
...
- 去背範圍固定為外框四向連通，不提供會直接扣除全圖相近色的選項
