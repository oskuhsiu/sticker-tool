...
7. Save Project ZIP V4 at any time, or build a target-specific LINE ZIP after all required current bytes
...
Background removal is never baked into V4 ingest. `none` preserves target-fitted raw pixels; color-keying is local;
...
Project ZIP V4 contains the product target, checksummed raw chunks, complete sample/visual indices,
drafts, current renders, selection/final-byte evidence, explicit color-key scope/edge settings, and implementation versions. It excludes source video, audio, model
...
entries directly to the project store. V3 color-key archives migrate to their historical all-matching/soft
semantics, rather than adopting the new default. V2 archives migrate to Animated Sticker. V1 archives import only
...
- **Solid-color key:** local Canvas operation with no model download. It alone exposes scope and edge
  controls. The new **outer-edge connected** default follows four-way candidate paths, preserving enclosed
  matching subject details but possibly leaving enclosed background holes; **all matching** removes those
  holes at the risk of punching through the subject. **Decontaminate** keeps a soft alpha edge while removing
  the keyed color from opaque composite edge pixels; **soft** can retain a colored halo; **hard** can create
  jagged or shrunken contours. Auto-detected green screens use the same choices with their dedicated
  greenness matte/despill. None, IMG.LY, local BiRefNet, and Colab do not show or consume these controls.
...
- `videoProjectZip.ts` owns asynchronous V4 export, strict streaming import, V2/V3 migration, and explicit V1 mapping.
...
npm run test:video          # V4 round-trip, V2/V3 migration, strict rejection, V1 mapping, render contracts
...
exact-target editing and controlled playback, round-trips Project V4 without a source decoder, verifies
