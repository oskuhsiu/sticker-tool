...
export function VideoSourceStep(props: {
  ...
  return (
    <div className="video-source-card">
      ...
      <Row>
        <Field label="輸出產品">
          <select value={props.target} onChange={(event) => props.onTarget(event.target.value as VideoOutputTarget)}>
        ...
        <Field label={props.target === 'animated-emoji' ? '縮圖來源第幾張' : '封面第幾張'}><input type="number" min={1} max={props.count} value={props.cover} onChange={(event) => props.onCover(Number(event.target.value))} /></Field>
      </Row>
      <Row>
        <Field label="專案預設去背">
      ...
      <p className="tab-desc">
        ...
        去背不在 ingest 執行；只在你產生單張預覽時處理實際候選格。
      </p>
